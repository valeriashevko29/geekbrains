# *Реализовать структуру данных «Товары». Она должна представлять собой список кортежей.
# Каждый кортеж хранит информацию об отдельном товаре. В кортеже должно быть два
# элемента — номер товара и словарь с параметрами, то есть характеристиками товара:
# название, цена, количество, единица измерения. Структуру нужно сформировать программно,
# запросив все данные у пользователя.
# Пример готовой структуры:
# [
# (1, {“название”: “компьютер”, “цена”: 20000, “количество”: 5, “eд”: “шт.”}),
# (2, {“название”: “принтер”, “цена”: 6000, “количество”: 2, “eд”: “шт.”}),
# (3, {“название”: “сканер”, “цена”: 2000, “количество”: 7, “eд”: “шт.”})

# Нужно собрать аналитику о товарах. Реализовать словарь, в котором каждый ключ —
# характеристика товара, например, название. Тогда значение — список
# значений-характеристик, например, список названий товаров.
# Пример:
# {
# “название”: [“компьютер”, “принтер”, “сканер”],
# “цена”: [20000, 6000, 2000],
# “количество”: [5, 2, 7],
# “ед”: [“шт.”]

num = 0
goods = []
features = {'название': '', 'цена': '', 'количество': '', 'ед': ''}
analytics = {'название': [], 'цена': [], 'количество': [], 'ед': []}
while True:
    if input('Для продолжения введите Enter, для окончания введите q').lower() == 'q':
        break
    num = num + 1
    f_copy = features.copy()
    for f in features:
        val = input(f'Введите значение свойства {f}: ')
        f_copy[f] = int(val) if f in 'цена количество' and val.isdigit() else val
        analytics[f].append(f_copy[f])
           # здесь создается пара ключ-значение. Т.е. f_copy - это словарь, [f] - ключ, к которому
        # мы хотим добавить значение в словарь, val - значение, которое мы добавляем.
    goods.append((num, f_copy))
print(f'Структура товаров: \n{goods}')
# print(f'Аналитика товаров: \n{analytics}')
for key, value in analytics.items():
    print(f'{key}: {value}')
